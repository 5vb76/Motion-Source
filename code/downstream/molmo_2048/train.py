"""Continue Molmo training on 2,048 QA examples and 512 source examples.
...
from soft_query_backend import assemble_soft_query_backend, VideoQuery20Request
...
def source_training_request(backend, row, source_worker):
    ...
    mask = torch.stack(
        [
            box_grid_occupancy(
                np.array(box) / [width, height, width, height], 9, 9, device="cuda"
            )
...
def main():
    ...
    try:
        ...
        with pilot.load_runtime(
            ...
            load_method_parameter_state(
                method_parameters, payload["method_parameter_state"]
            )
            del payload
            ...
            for position in range(step * 8, 2048):
                ...
                loss = training_runtime.native_loss(backend, request, answer)
                ...
                if position % 4 == 0:
                    ...
                    with source_training_request(
                        ...
                        loss = training_runtime.native_loss(
                            backend,
                            source_request,
            ...
            metrics = {}
            # Fixed train probe and both dev panels; no checkpoint selection.
            probe = qa_rows[:64]
            metrics["train64"] = evaluate(backend, probe, gold_by_id, "train64")
            # LOCAL_PATH: Supply qa_dev650.json beside this script for development-set evaluation.
            metrics["dev650"] = evaluate(
                backend, read_json(RUN_DIR / "qa_dev650.json"), gold_by_id, "dev650"
            )
