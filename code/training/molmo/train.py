"""Train Molmo through F/G/R and compare validation endpoints."""
...
from evaluation import evaluate, checkpoint_rank
...
def main():
    ...
    try:
        ...
        g_metrics = json.loads(
            (RUN_DIR / "validation/G_endpoint/metrics.json").read_text()
        )
        r_metrics = json.loads(r_metrics_path.read_text())
        selected = (
            "R" if checkpoint_rank(r_metrics) > checkpoint_rank(g_metrics) else "G"
        )
        selected_metrics = r_metrics if selected == "R" else g_metrics
        selected_path = RUN_DIR / f"checkpoints/{selected}_endpoint.pt"
        write_json(
            RUN_DIR / "SELECTED.json",
            {
                ...
                "selection": "new val only; G vs R endpoints; tie earlier G",
            },
        )
        # LOCAL_PATH: This plan entry must resolve to a local checkpoint with the recorded hash.
        old_comparator = plan["old_comparator"]
        assert file_sha256(old_comparator["path"]) == old_comparator["sha256"]
        old_checkpoint = torch.load(
            old_comparator["path"], map_location="cpu", weights_only=False
        )
        load_method_parameter_state(
            adapter.method_parameters, old_checkpoint["method_parameter_state"]
        )
        del old_checkpoint
        old_metrics_path = (
            RUN_DIR / "validation/old_checkpoint_same_new_val/metrics.json"
        )
        if not old_metrics_path.exists():
            evaluate(
                adapter,
                MotionDataset(validation_rows, worker),
                old_metrics_path.parent,
                "old_checkpoint_same_new_val",
            )
        old_metrics = json.loads(old_metrics_path.read_text())
        comparison = {
            "new": selected_metrics,
            "old_on_same_new_val": old_metrics,
            "delta_percentage_points": {
                k: 100 * (selected_metrics[k] - old_metrics[k])
                for k in [
                    "joint_state_accuracy",
                    "source_macro_accuracy",
                    "strict_four_line_exact",
                ]
            },
            "per_source_delta_percentage_points": {
                s: 100
                * (
                    v["joint_state_accuracy"]
                    - old_metrics["per_source"][s]["joint_state_accuracy"]
                )
                for s, v in selected_metrics["per_source"].items()
            },
            "next_action": "Report to user; other models require user decision",
            "other_models_started": False,
            "historical_exposure": "new val reuses historical development; comparative diagnostics, not fresh independent test",
        }
        ...
        write_json(
            RUN_DIR / "STATUS.json",
            {"state": "complete_awaiting_user_review", "updated_at_unix": time.time()},
        )
    finally:
        worker.close()
